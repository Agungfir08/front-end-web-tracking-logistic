...
function ShipmentInfo() {
    ...
    return (
        <div
            data-test="shipment-info"
            className="bg-white rounded-lg p-6 flex flex-col justify-center sm:w-[310px] sm:h-[400px] w-[375px] h-[440px] mt-1">
            <div className="flex flex-col gap-4">
                <div className="flex flex-rows gap-24">
                    {infoItems.slice(0, 2).map((item, index) => (
                        <InfoItem
                            dataTest={`shipment-info-child${index}`}
                            key={index}
                            name={item.name}
                            value={item.value}
                        />
